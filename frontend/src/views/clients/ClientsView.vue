<template>
    <v-container fluid>
      ...
      <v-dialog v-model="addDialog" max-width="600">
        <v-card title="Add New Client">
          <v-card-text>
            <v-form ref="addFormRef" v-model="addValid" >
              <v-text-field v-model="newClient.clientName" label="Client Name" :rules="[rules.required]"></v-text-field>
              <v-text-field v-model="newClient.cnic" label="CNIC" :rules="[rules.required]"></v-text-field>
              <v-text-field v-model="newClient.phone" label="Phone"></v-text-field>
              <v-text-field v-model="newClient.email" label="Email" :rules="[rules.email]"></v-text-field>
              <v-text-field v-model.number="newClient.discount" label="Discount" type="number" step="0.25" min="0" max="20" :rules="[rules.discount]"></v-text-field>
          ...
          <v-card-actions>
            ...
            <v-btn color="success" @click="saveNewClient" :loading="store.loading">Save</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
  ...
  <script setup lang="ts">
    ...
    const newClient=ref<Omit<Client,'id'>>(
      {
        clientName:'',
        cnic:'',
        phone:'',
    ...
    async function saveNewClient(){
      ...
      try{
        ...
        showNotification('Failed to create the client','error')
    ...
    async function confirmDelete() {
      try{
        ...
        showNotification('Failed to delete the client','error')
    ...
    async function handleUpdateClient(updatedClient: Client) {
      try {
        await store.updateClient(updatedClient.id, updatedClient)
        // clientDetailsRef.value?.toggleEdit()
